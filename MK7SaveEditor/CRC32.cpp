...
{
	return crc32buf(buf, strlen(buf));
}
...
{
	uint32_t x = crc32str(s);
	return find_old_crc(x, c);	
}
 
uint32_t compute_removed_str(const char *s, const char *s2)
{
	int x = strlen(s2)-1;
	uint32_t now = crc32str(s);
	while (x>=0) {
		now = find_old_crc(now, s2[x]);
		x--;
	}
	return now;
